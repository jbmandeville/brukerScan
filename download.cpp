...
{
    int nChecked=0;
    bool foundAnatomy=false;  bool foundUMap=false;
    for (int jList=0; jList<_scanItems.count(); jList++)
    {
        if ( _scanItems[jList].checkState() ) nChecked++;
        downloadScan scan = _scans.at(jList);
        FUNC_INFO << jList << _scanItems[jList].checkState() << (scan.category == category_anatomy) << (scan.category == category_MRAC) << scan.dim.t << scan.existsOnDisk;
        scan.isAnatomy = _scanItems[jList].checkState() && (scan.category == category_anatomy)   && (scan.dim.t == 1) && (scan.existsOnDisk);
        scan.isMRAC        = _scanItems[jList].checkState() && (scan.category == category_MRAC) && (scan.dim.t == 1) && (scan.existsOnDisk);
        foundAnatomy   |= scan.isAnatomy;
        foundUMap |= scan.isMRAC;
        _scans[jList] = scan;
    }
    FUNC_EXIT << nChecked << foundAnatomy << foundUMap;

    return (nChecked == 2) && foundAnatomy && foundUMap;
}
...
//        QStringList stringList = line.split(QRegularExpression("\\s+"));
        ...
        {
            scan.category = category_anatomy;
            scan.categoryName = "anatomy";
        }
...
//        _queryDownloadGroupBox->setEnabled(false);
    ...
    {
        QString fileName = "anatomy/" + _anatomyDirBox->currentText() + "/raw.nii-infodump.dat";
...
void MainWindow::outputDownloadList()
{
    ...
    for (int jList=0; jList<_scans.size(); jList++)
    {
        ...
        if ( scan.selectedForDownload )
        {
            ...
            else if ( scan.category == category_anatomy )
                out << scan.scanNumber << " anatomy dicom raw.nii \n";
