...
void MainWindow::createAnatomyPage()
{
    ...
    // input directory
    ////////////////////////////////////////////////
    auto *anatDirLabel = new QLabel("Sub-directory on 'anatomy'");
    auto *generateLabel = new QLabel("Generate NIFTI file from 2dseq");
    auto *anatInputFileLabel  = new QLabel("file name ",_anatomyPage);
    _anatomyDirBox = new QComboBox();
    _anatomyFileBox  = new QComboBox();
    _generateNifti = new QPushButton("2dseq --> raw.nii");
    _reorderRaw = new QCheckBox("reorder echoes");
    _reorderRaw->setChecked(false);
    ...
    connect(_generateNifti, SIGNAL(pressed()), this, SLOT(generateBrukerNiftiAnatomical()));
    ...
    auto *convert2dseqLayout = new QHBoxLayout();
    convert2dseqLayout->addWidget(generateLabel);
    convert2dseqLayout->addWidget(_generateNifti);
    convert2dseqLayout->addWidget(_reorderRaw);

    _convert2dseqWidget = new QWidget();
    _convert2dseqWidget->setLayout(convert2dseqLayout);

    auto *inputLayout = new QVBoxLayout();
    inputLayout->addLayout(inputFileLayout);
    inputLayout->addWidget(_convert2dseqWidget);
...
void MainWindow::openedAnatomyPage()
{
    ...
    QDir const anatomyTopDir("./anatomy");
...
void MainWindow::updateAnatomyFileName()
{
    ...
    QString path = "./anatomy/" + _anatomyDirBox->currentText();
//    QString path = "./anatomy/" + _anatomyDirBox->itemText(indexInBox);
...
{    // use jip to create the MC template, and then go on to the next step
    ...
    if ( anatomical )
        connect(process, SIGNAL(finished(int, QProcess::ExitStatus)),
                this, SLOT(finishedGenerateBrukerNiftiAnatomical(int, QProcess::ExitStatus)));
    else
        connect(process, SIGNAL(finished(int, QProcess::ExitStatus)),
...
void MainWindow::extractFreeSurferOverlays()
{
    ...
    QString brainName = "anatomy/" + _anatomyDirBox->currentText() + "/brain.nii";
    QString atlasName = "anatomy/" + _anatomyDirBox->currentText() + "/atlas.nii";
    QString comName   = "anatomy/" + _anatomyDirBox->currentText() + "/align.com";
    QString outputDir = "anatomy/" + _anatomyDirBox->currentText() + "/templateOverlaysFromFreeSurfer";
...
void MainWindow::alignAnatomyToTemplate()
{
    ...
    QString inputFileName = "anatomy/" + _anatomyDirBox->currentText() + "/" + _anatomyFileBox->currentText();
...
bool MainWindow::anatomyFileExists(QString dirName, QString fileName)
{
    ...
    QString fullName = "anatomy/" + dirName + "/" + fileName;
...
void MainWindow::enableAnatomyActionButtons()
{
    ...
    fileName = "anatomy/" + _anatomyDirBox->currentText() + "/templateOverlaysFromFreeSurfer";
...
void MainWindow::displayAnatomy()
{
    ...
    QString inputFileName = "anatomy/" + _anatomyDirBox->currentText() + "/" + rootFileName;
    QStringList arguments;
    arguments.append(inputFileName);
    ...
    if ( !rootFileName.compare("align.nii") )
    {
        ...
        QString ovlListName = "anatomy/" + _anatomyDirBox->currentText() + "/templateOverlaysFromFreeSurfer/overlay-list.dat";
    ...
    else if ( !rootFileName.compare("brain.nii") )
    {
        ...
        {
            QString atlasFileName = "anatomy/" + _anatomyDirBox->currentText() + "/atlas.nii";
            arguments.append("-A");
